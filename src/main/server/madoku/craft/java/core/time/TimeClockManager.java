...
public final class TimeClockManager {
	...
	/**
	 * Resolves the next overworld time value used by ServerLevel.tickTime().
	 * Keeping the fractional part here lets configured rates below one tick per
	 * server tick progress without repeatedly rounding back to the current time.
	 */
	public static long resolveNextWorldTime(ServerLevel level, long vanillaNextDayTime) {
		...
		if (hasObservedWorldTime && level.getDayTime() < lastObservedWorldDayTime) {
			worldTimeRemainder = 0.0D;
		}

		float rate = resolveWorldClockRate(server);
		...
		return wholeTicks <= 0L
			? level.getDayTime()
			: safeAdd(level.getDayTime(), wholeTicks);
	}
	...
	/** Minecraft 1.21.11 has no clock-marker registry to remap. */
	public static long resolveConfiguredTimeMarkerTicks(Object markerKey) {
		return -1L;
	}
	...
	private static void observeRuntimeState(MinecraftServer server) {
		...
		observeGameplayTicks(getGameplayTicks());
		observeWorldTime(overworld.getDayTime());
	}
