...
public final class SeasonEnvironmentTransitionAPIManager {
	...
	private static final SeasonAPIManager.Season[] SEASONS = SeasonAPIManager.Season.values();
	...
	private static EnvironmentTransitionConfigAPIManager.Settings lastUpdatedSettings;
	private static SeasonConfigManager.Settings lastUpdatedSeasonSettings;
	private static SeasonAPIManager.SeasonState lastUpdatedState;
	...
	private SeasonEnvironmentTransitionAPIManager() { }

	public static void initialize() {
		EnvironmentTransitionConfigAPIManager.initialize();
	}
	...
	static void updateSeasonState(SeasonAPIManager.SeasonState state) {
		if (state == null || state.season() == null) return;
		EnvironmentTransitionConfigAPIManager.Settings settings = EnvironmentTransitionConfigAPIManager.getSettings();
	...
	public static double adjustTemperature(double base, String season) {
		return adjustTemperature(base, season, TimeAPIManager.getCurrentAbsoluteDayTime());
	}
	...
	public static SeasonBiomeClimateAPIManager.Climate adjustForShelter(
		LevelReader level,
		BlockPos pos,
		SeasonBiomeClimateAPIManager.Climate climate
	) {
		if (climate == null) {
			return climate;
		}
		...
		return new SeasonBiomeClimateAPIManager.Climate(temperature, humidity);
	}
	...
	public static boolean isWeatherTransitionEnabled() {
		return SeasonAPIManager.isEnabled() && SeasonBiomeClimateAPIManager.isTemperatureEnabled() && SeasonBiomeClimateAPIManager.isHumidityEnabled()
			&& EnvironmentTransitionConfigAPIManager.getSettings().weatherEnabled();
	}

	public static boolean isWaterTransitionEnabled() {
		return SeasonAPIManager.isEnabled() && SeasonBiomeClimateAPIManager.isTemperatureEnabled()
			&& EnvironmentTransitionConfigAPIManager.getSettings().waterEnabled();
	}

	public static boolean isTemperatureTransitionEnabled() {
		return SeasonAPIManager.isEnabled() && SeasonBiomeClimateAPIManager.isTemperatureEnabled()
			&& EnvironmentTransitionConfigAPIManager.getSettings().seasonTransitionsEnabled()
			&& EnvironmentTransitionConfigAPIManager.getSettings().temperatureEnabled();
	}

	public static boolean isHumidityTransitionEnabled() {
		return SeasonAPIManager.isEnabled() && SeasonBiomeClimateAPIManager.isHumidityEnabled()
			&& EnvironmentTransitionConfigAPIManager.getSettings().seasonTransitionsEnabled()
			&& EnvironmentTransitionConfigAPIManager.getSettings().humidityEnabled();
	}
	...
	public static Biome.Precipitation resolvePrecipitation(Biome biome, String season) {
		...
		return resolvePrecipitation(SeasonBiomeClimateAPIManager.resolve(biome), season);
	}

	public static Biome.Precipitation resolvePrecipitation(SeasonBiomeClimateAPIManager.Climate climate, String season) {
		return resolvePrecipitation(
			climate,
			season,
			temperatureOffset,
			humidityOffset,
			TimeAPIManager.getCurrentAbsoluteDayTime());
	}
	...
	private static double resolveDailyTemperatureModifier(long absoluteDayTime) {
		return resolveDailyTemperatureModifierAtMinutes(TimeAPIManager.getTotalMinutes(absoluteDayTime));
	}
	...
	public static boolean shouldFreezeAt(LevelReader level, BlockPos pos, SeasonBiomeClimateAPIManager.Climate climate) {
	...
	public static boolean shouldMeltAt(SeasonBiomeClimateAPIManager.Climate climate) {
	...
	private static double resolveSmoothSeasonalOffset(
		java.util.Map<String, EnvironmentTransitionConfigAPIManager.Adjustment> adjustments,
		SeasonAPIManager.SeasonState state,
		int count,
		int adjustmentCount,
		int timeRateDays,
		...
		if (count >= availableStages && state.seasonDay() >= currentStageStartDay) {
			SeasonAPIManager.Season nextSeason = nextSeason(state.season());
	...
	private static double resolveCumulativeOffset(
		java.util.Map<String, EnvironmentTransitionConfigAPIManager.Adjustment> adjustments,
		SeasonAPIManager.Season season,
		int currentCount,
		int adjustmentCount
	) {
	...
	private static double signedValue(
		...
		EnvironmentTransitionConfigAPIManager.Adjustment adjustment = adjustments.get(season);
