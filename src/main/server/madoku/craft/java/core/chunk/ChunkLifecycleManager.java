...
final class ChunkLifecycleManager {
	...
	static void initialize() {
		ServerChunkEvents.CHUNK_LOAD.register((level, chunk) -> onChunkLoad(level, chunk, false));
	...
	private static void onChunkUnload(ServerLevel level, LevelChunk chunk) {
		...
		ChunkAPIManager.removeChunk(ChunkAPIManager.levelId(level), chunk.getPos().toLong());
