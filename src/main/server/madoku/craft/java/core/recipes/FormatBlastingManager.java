...
public final class FormatBlastingManager {
	...
	static Recipe<?> buildOverride(JsonObject root, BlastingRecipe recipe, String resultItemId, int resultCount) {
		...
		return new BlastingRecipe(RecipesFormatManager.readRecipeGroup(root, recipe.group()), recipe.category(),
			input, result, recipe.experience(), recipe.cookingTime());
	}

	static void writeDefaults(JSONFormatAPIManager.ObjectBuilder root, BlastingRecipe recipe) {
